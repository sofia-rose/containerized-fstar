...
let abs1 (x: int_32): Pure int_32
  ...
  (ensures (fun res -> v res = abs (v x))) // enhance this post-condition to use abs above
...
= if x = min_int32 then None else Some (abs1 x)

/// Summing three numbers
let sum3 (x y z:int_32) : Pure int_32
  (requires ok (+) x y /\ ok (+) (x + y) z) //weaken the precondition to something more reasonable
